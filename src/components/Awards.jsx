    ...
    <section className="flex flex-col items-center justify-center min-h-screen text-base ">
      ...
      <div className="w-4/5  border-black capitalize">
        {awardsData.map((award, index) => (
          <div
            key={index}
            className="h-17 bg-amber-200 overflow-hidden cursor-pointer group"
          >
            <div className="relative h-40 bg-red-200 transform -translate-y-20 transition-transform duration-500 ease-in-out group-hover:translate-y-0">
              {/* First Row */}
              <div className=" bg-black text-[#fd2c2a] p-[1.77rem] flex items-center justify-center pl-12">
                <h1 className=" w-2/3">{award.name}</h1>
                {/* <h1 className="text-lg">{award.type}</h1> */}
              </div>
              {/* Second Row */}
              <div className="bg-[#fd2c2a] text-black p-[1.67rem] flex items-center justify-center pl-12">
